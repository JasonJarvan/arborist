...
⇒ **`session_id` 全局唯一那条根约束需要一个一等的例外表达。** 当前规范只有「一个会话属于一个项目」，于是**合法的跨仓可达性只能靠伪装成重复登记来实现**，而 validator 必然把它报成高危。**结论：这一族告警不能靠删数据消掉**；正确修法是给跨仓 peer 一个一等 schema 字段，并让 `duplicate-session-id` / `pane-ref-conflict` / `project-mismatch` 三项检查对**带该字段且自陈完整**的条目降级为 info（仍列出，便于审计），对**不带标记**的保持高危。该字段落定前，validator 的高危报告对镜像条目是**已知假阳性**，处置一律是「出裁定、不删」。

**残留的真实风险（不因是镜像而消失）**：镜像两条共享同一 `pane_ref`，所以**当前**投递会落到同一个真实目标、**不会**打进第三方；但该 pane 被后续 session 复用之后，validator 报的「静默误投」就会变成真的。⇒ 镜像条目的 `pane_ref` 必须与权威条目同步失效，不能各自留存。
...
- **✅ tmux transport 的两条缺口已由一次人机同场 smoke test 关闭（2026-08-06 实测）**——原文两条分别是「不抢焦点只证到服务端状态层、未证到附着 client」与「提交键语义未在真 ATUI 上验证」。测法与逐格读数：
  ...
  > ⚠️ **这次成功不得被读成「tmux 解决了 composer 不提交那个痛点」。** 变量没有控制在痛点条件上：本次是**短消息 + 目标 idle**，而这两项**此前已各自被证明是成功的充分条件**（同一现象在另一复用器上、短文本 + 足够窗口时也成功）。⇒ 本次只证「tmux 下提交键语义照旧」与「不抢焦点」，**未**证任何关于长文本/活跃目标的事。要证那个需要在**长信封 + 活跃目标**下重测。
...
- **五种「注册表看不出来的投不进 / 投不对」（全部已实测 · 一致性 validator 也查不出 · 别指望注册表告诉你 · 覆盖度的如实表述见本条末，勿写成「病因已覆盖」）**：注册表一致性（§2.2.1 / §4）能排除**归属错**与**寻址错**，但下面这些形态在注册表里**一模一样**——`state=active`、`pane_ref` 有效、`session_file` mtime 新鲜、一致性检查全绿——投递却到不了目的地，或**到了但不是你想说的话**：
  ...
  **读表前必读（二）—— 分类的输入是「可观察签名」，不是底层病因。** 任何分类器（人或代码）**看不见**目标的鉴权状态、看不见沙箱边界、看不见 composer 内部；它只看见三类签名：**屏幕形态**（读屏文本）、**写入/提交命令的 stdout**、**transcript 里有无越界 nonce**。下表末二列正是照这条区分开的：能不能造出**签名**（可以，五格都能）与「该签名是否**只可能**由这一格的病因产生」（多数**未证明**）是两件事。
  ...
  >
  > ⇒ **启动侧不变量**：派生一个 ATUI 之后，**在它自登记完成之前不得向它投递**。这条同时解释了一个此前没人归因的失败类 —— 新起 ATUI 存在一个**就绪窗口期**，期间任何投递都会落进虚空，而读数与「目标坏了」不可区分。（启动方自身的「等二进制就绪」重试只保护启动方，**不保护投递方**。）

  > **为什么 `unclassified` 是硬要求**：**没有这一格，分类器会被迫把新病因塞进最像的已知格里**——那不是分类，是掩盖。本轮已经**三次**证明这件事会发生：形态数从三扩到五、再扩到六，两个新格（验证太早、构造侧损坏）在被发现前，其签名都曾被归进已有的解释里（前者被读成「目标在排队」，后者干脆全绿无人怀疑）。⇒ 落 `unclassified` 是**正当结论**，不是失败；把它硬塞进第 1–5 格才是。
  ...
  > **⚠️ 五格的独占性是「假定」，不是「已证明」**：独占性（这个签名**只可能**由这一格的病因产生）建立在「**已知病因集合完备**」这个假设上，而该假设在本轮**被推翻过三次**（三格 → 五格 → 六格）。⇒ 第 1–4 与 6 格的映射标 `假定`：签名匹配只说明「与该形态一致」，**不构成**「病因即此」。要把某格升为 `已证明`，得给它一条**像形态 5 那样直接判定**的判据（不依赖排除法）。
  ...
  - **本表覆盖度的如实表述（措辞是规范性的，不得简写）**：
    ...
    > **覆盖 6 种已知形态，每格有签名级 fixture；其中 1 格的触发模式常量仅凭单次观察，且 5 格的「签名→病因」映射独占性依赖「已知病因集合完备」这一已被三次推翻的假设。**
...
| 句柄形态 | 改名安全吗 |
|---|---|
| 句柄**含 session 名**（如 `{multiplexer, session, pane_id}` 三元组） | **不安全** —— 改名即让既有 `pane_ref` 腐烂（§3 已实测这一类：句柄静默失效、注入喷进虚空） |
| 句柄是**全局唯一且终身不变的 pane id**（不含 session 名） | **安全** —— 改名对寻址零影响 |

⇒ 两段式命名**只能在后一种形态上使用**。这也是为什么「换复用器」的评估必须逐格看句柄形态，而不是只看投递命令。
...
⇒ **必须显式清理。但「没有客户端就销毁」这个选项是错的手段** —— 它是**推论**而非**因果**：它只看「当下有没有客户端」，因此**分不出**「人类关掉了窗口」与「人类只是暂时离开」，而这两者的正确处置**相反**。

**正确手段 = 捕获关闭事件本身（已实测，逐格）**：

| 人类动作 | 内层复用器该怎样 | 机制与读数 |
|---|---|---|
| **关闭外层 tab / pane** | **一并结束** | 该 pane 内的前台进程收到 **`SIGHUP`**（实测，可捕获）。**但据此做 `trap` 清理只在「脚本 shell 承载」时有效，在真实用法下失效 —— 见下方警告。** |
| **detach 外层 session**（整个会话挂起、稍后重入） | **保留、继续运行** | pane 未被销毁 ⇒ 前台进程**不收信号** ⇒ trap 不触发。**实测**：外层客户端断开后，外层 session 与内层 session **均存活** |
| **detach 内层 session** | **保留、可重入** | attach 命令**正常返回** ⇒ 走 trap 之外的路径 ⇒ 不销毁 |
| ATUI 自己退出 | 自然结束 | 内层 session 随之结束、attach 返回 |

⇒ 该手段比「没有客户端就销毁」精确得多，并**顺带把内层 detach 变成合法操作**（保留、可重入），而不是「误按一次就杀掉一个正在工作的 agent」。
...
>
> ⇒ **可靠手段回到「最后一个客户端断开即销毁 session」这个复用器内建选项**：它**不依赖信号、不依赖 shell 类型**，因此对「脚本承载」与「交互 shell 粘命令」**同构**。它的已知代价是**内层 detach 会被误伤**（内层 detach 也让 session 变成无客户端）；该代价是否可接受，取决于人类用的是**哪一层**的 detach —— 用外层 detach 的人不受影响。
>
> **通则（这条比结论重要）**：**验证一个「由人类操作触发」的机制时，测试的进程结构必须与人类的真实用法同构。** 本例中「信号确实到达」与「trap 确实执行」是**两个不同的命题**，测第一个通过并不能推出第二个 —— 而中间那一步差异（脚本 shell vs 交互 shell）在测试里是不可见的。同族见 [`verification-and-gates`](./verification-and-gates.md#新增的观测动作必须先证明它不扰动被观测者)：**观测/测试的结构本身会决定它能看见什么。**
...
**实测边界（如实记）**：以上读数取自一个**独立的测试会话**（不触碰任何真人终端），逐格记录「动作 → 内层 session 是否存活」。**未验证**：其它复用器实现是否同样在关闭 pane 时发 `SIGHUP` —— 该信号不是标准要求，而是该实现的**观测行为**；换实现必须重测（同 §3「每种复用器都有一个看起来最自然却静默回落的命令」那条通则）。
...
**根规则**：**复用器环境变量只是「待证线索」，不是身份证明。** 必须用**独立的第二条证据**佐证（复用器侧列出该 pane 的**运行命令**与**工作目录**，与本会话声称的身份/cwd 比对），矛盾则 **fail closed、不登记**。

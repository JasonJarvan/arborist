...
⇒ **必须显式设成后者**；默认值会制造「看不见的活体」。两条实施注意：
- **设置时机**：先以 detached 方式创建、再打开该选项，会让 session **当即自我销毁**（当时没有任何客户端）。⇒ 顺序必须正确（创建即 attached，或在已有客户端之后再设）。
- **detach 快捷键的风险取决于人类的实际习惯**：若人类是**关闭外层 tab** 来结束 ATUI，则该选项与其习惯一致、无额外风险；若人类习惯 detach 后再回来，则该选项会把 detach 变成「杀掉」⇒ 那种用法下必须改用别的清理机制（例如退出时回收，而非 unattached 即销毁）。**先问清人类怎么关，再选机制。**

...
- **⚠️ 随发 adapter 的契约缺口（截至 2026-08-05，必须可见——契约不得被读成「代码已做到」）**：随发 `scripts/agenttui.py` **尚未**实现下列契约条款，port 任务另行追踪；在它补齐前，调用方须自行承担对应风险：
  ...
  - **⚠️ 随发 adapter 不支持跨项目投递 —— 而这一条会让整套契约在跨项目场景下全部失效**（2026-08-06 实测）：`load_agent(repo, name)` **只在单个仓**的 `.arborist/agents/<name>/` 下找 leaf，而仓根是**单值**参数 ⇒ 发送方与目标**必须同仓**。全局 `index.json` 里**有** `projects[].path`，所以跨项目寻址在**数据上是可行的**，只是 adapter 没有用它。

    **后果不是「少一个便利功能」，而是一条系统性绕过**：跨项目投递只能**手工 raw 注入**，而手工注入**绕过全部契约** —— 无规则 5 存在性 preflight、无规则 6 能力检查、无七值结果模型、无规则 8 的 ack 消费、无成帧写入、无验证窗口修复、无焦点抢夺记录。**每一条今天做的加固，在跨项目投递上都不生效。**

    > **归因（这决定修法，所以必须写清）**：已实测的一次跨项目投递里，投递方对一个空闲目标用了**入队键**而非提交键，违反规则 1。把它记成「**某人的路由错误**」是错的归因 —— 手工注入者**必须自己重新实现路由**（活性判定、brand 键映射、间隔、验证），而契约有六条规则，人在现场逐条正确实现的概率很低。⇒ **这类错误是缺口的产物，不是疏忽的产物。** 记成疏忽 ⇒ 下一个人照样犯；记成缺口 ⇒ 去补缺口。
    >
    ...
    **过渡期的唯一诚实处置**：跨项目投递前**明确记下自己绕过了哪些规则**，并**逐条手工执行**（至少：按目标真实活性选键、注入后**等 composer 就绪再提交**、验证 nonce 越界命中、失败不盲目重发）。**不得**因为「手工投递成功过」就认为契约已被满足。

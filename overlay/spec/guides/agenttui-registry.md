...
- **参考 adapter（随发 · opt-in · 二选一别混）**：
  ...
  - **zellij 侧读数总表（对应契约规则 5）——附原始读数，勿只记结论**。下表每格都是同一版本（`zellij 0.44.3`）上的直接观测，**rc / stdout / stderr 三者分开记**：
    ...
    > **⚠️ 一处未解决的读数矛盾（不得据 rc 判定探针成功，直到它被解决）**：对**已聚焦**的 pane 跑 `focus-pane-id`，两份独立报告不一致——一份为 `rc=0` + 空输出，另一份为 `rc=2` + `already focused`。本仓**未独立测**该格：测它必然抢焦点、会打断人类。⇒ 在该格被测准前，探针**不得**把「rc 非零」当作拒绝依据——若第二份报告为真，那会把**最常见**的「目标已聚焦」情形误判为不可达。当前实现只按合并文本判定，是这条约束下的正确保守做法。

...
→ 「没被看到 → 进不去」，这是最强的机械保证（**白名单 > 黑名单**；这条通则本身连同它的配套那半——「批准必须便宜」——见 [`verification-and-gates`](./verification-and-gates.md#allowlist-over-denylist通则门问的是谁批准了不是它有什么问题)）。

## 范例清单 vs 穷举清单（exemplary vs exhaustive）

同步与隐私这两件事都靠清单：同步 allowlist、结构性排除、占位符表、检测层的模式清单。而**一份清单是范例还是穷举，必须写在清单上**——因为两者要求的**读者动作相反**：

| 清单类型 | 「不在清单里」意味着 | 读者要做什么 |
|---|---|---|
| **穷举**（exhaustive）| 它**不属于**这一类，作者已承担完整性 | 什么都不用做 |
| **范例**（exemplary）| **没人替你判过** | **必须**自己按判据判一次 |

**失效模式是单向的**：**范例清单被当穷举用** ⇒ 清单外的东西被读成「已裁定为安全」，而它其实只是**没被想到**——于是那次本该发生的判断静默消失。反向（穷举被当范例）只会多做一次无用判断，无害。⇒ 缺省必须偏保守：**未标注的清单一律读作范例**；作者想让读者省掉那次判断，就得**显式声明穷举**并承担完整性。

本篇两份清单据此标注：

- 「[绝不进入被同步文件的东西](#绝不进入被同步文件的东西结构性排除机械可保证)」里的**同步 allowlist 是穷举**——它是同步机制的**实际输入**，白名单之外机制物理上碰不到，完整性由机制本身保证；
- 同节下面那串**被排除项是范例**（个人身份 / 个人 WIP / 运行时机器态 / 内部协调 / 密钥…）——它举的是**已知形态**，新形态照上面那条授权判据（「另一个仓 clone 这份 guide，这个值还成立吗？」）自己判一次。

**与防御纵深的关系**：下一节第 2 层（正则检测）**天生只能穷举可枚举的模式**，语义残余必须留给第 3 层（模型 / 人）。把第 2 层的模式清单读作穷举，正是「靠 regex 抓全」那个错误的另一种写法——那也是为什么第 3 层在公开边界上**不可省**。

...
### 取证命令本身会出错，而它的错以正常读数的形式呈现

**上一条讲记录的那一格是假的；这一条讲产生那一格的那条命令是假的。**

> **一条我没验证过正确性的诊断命令，会产出一个看起来精确的错误读数。**

**为什么它比被诊断的缺陷更难发现**：诊断命令的产物**自带可信外观**——一个数字、一个空 diff、一句「零命中」。读者拿它当**事实层**，而它其实也是**推断层**（推断「这条命令测的是我以为的那件事」）。

**一晚内的实测清单（五例、两个独立当事人 —— 这是它能立为通则而非轶事的理由）**：

| 命令形态 | 实际发生了什么 | 差点/已经得出的错误结论 |
|---|---|---|
| `for p in "a b"; do set -- $p` | 位置参数未分词，实际执行 `git diff "a b" ""` | 「那个 commit 不存在 / diff 非空」（实为 0 字节、结论相反）|
| `<诊断命令> >/dev/null 2>&1` | **被诊断的子命令根本不存在**，`2>&1` 使其失败**从未可见** | 「每次都跑了」（实为整晚一次都没跑成）|
| `<读数> \| tail -2; echo $?` | `$?` 取的是**管道末端**的退出码 | 「rc=0，通过」（实为 rc=5，拒绝）|
| 目录清单 `ls` 读数不完整 | 输出被截断/不全 | 「**当前任务目录不见了**」——一个**不存在的险情** |
| 一次投递诊断丢弃 stderr | 同第 2 例 | 诊断被吞 |

**这条不能以纪律形式存在（本节最重要的一句）**：

> 上表第 1、2、5 例出自**同一当事人**，而它**三次都知道这条规则、三次都没触发它**。
>
> ⇒ **一条规则若在当事人已知的情况下仍反复不触发，它的形式就是错的，不是它的内容。** 该规则必须被改造成**机械物**（不可省的默认、会失败的检查、或一个不提供出错形态的接口），而不是又写一遍。**再写一遍是最诱人也最无效的处置** —— 它产生「已处理」的读数而不改变发生率。

**机械产物（三条，都是「不提供出错形态」而非「提醒别出错」）**：

1. **诊断命令不得丢弃 stderr。** `>/dev/null 2>&1` 加在一条**用来判断状态**的命令上，等于把它的失败与它的否定结论**压成同一个读数**。要静默就只静默 stdout，**stderr 与退出码必须留**。
2. **退出码必须取自被诊断的那条命令**，不是管道末端。先把输出存进变量，再取 `rc`。
3. **首次跑就得到否定结论时，必须先证命令自身**（换一个已知为真的输入跑一次，看它是否给出肯定结论）。理由：**否定结论最容易被当成发现而不是被复核** —— 它自带解释力，而「我的命令写错了」不自带解释力。

**判「够不够机械」的尺子（这条把上面那个元结论变成可施加的判据）**：

> **一条规则的机械化程度 = 省略它需要多做的动作。**
> 省略它需要**显式加一个参数 / 显式写一句绕开** ⇒ **门**；
> 省略它**什么都不用做**（默认就跳过）⇒ **零**，写多少遍都一样；
> **正确做法比错误做法更费事** ⇒ **负**。

**负的那一档必须优先造载体**，理由是它不稳定：零机械化的规则只是**不生效**，而负机械化的规则**会被时间稳定地推向错误一侧** —— 每一次赶时间都往错的方向走一格，而那一格看起来像是省事，不像是违规。

拿这把尺子量上面三条产物，**其中两条当场不合格**（这正是尺子有用的证据——它否掉的是刚写下的东西）：

| 产物 | 省略它要多做什么 | 判定 |
|---|---|---|
| ① 不得丢弃 stderr | 要**显式写** `2>&1` | **够格** |
| ② 退出码取自被诊断命令 | **什么都不用做**，而且 `cmd \| tail; echo $?` 是**更省事**的写法 | **负**（不只是零）|
| ③ 首次否定结论先证命令 | **什么都不用做** | **不够格（最弱）** |

⇒ ②③ 的机械载体是 `scripts/probe.py`：它把两条流**分开**保留、报**被诊断命令自己**的 rc，并且——最吃劲的一条——**拒绝单独交出一个否定读数**。要拿到否定结论，必须**另给一条已知读正的 control**（`--control-file`，argv 逐行，不经 shell），或**显式写下**「我接受一个未经佐证的否定，理由是……」（`--accept-unverified-negative`）。两条都要付一个动作。四种结局各有**互不相同**的退出码：`positive` / `negative`（已佐证）/ `inconclusive`（扣住）/ `probe-suspect`（control 也读负 ⇒ **有问题的是 probe，不是被测物**）。**「东西不在」与「我判不出它在不在」共用一个读数，就是本节要消除的那次坍缩。**

**三种伪装的清单不放在这里，放在 `scripts/probe.py` 的文档里**（命令写错 / 流被吞 / 数据在传给解析器的路上被改写；共同点是**失败都披着「被测物有问题」的外衣**）。理由是形式而非内容：**清单可以被逐条对照，正文只能被读到** —— 而这三种正是要在动手前逐条排除的东西。本节留判据，清单跟着工具走。

**第 6 例（写这一节时新撞的，我的第 3 次）**：把 probe 的 JSON 经 `echo "$out"` 回显给解析器——**某些 shell 的 `echo` 会解释反斜杠转义**，`\n` 变成真换行，JSON 当场非法。⇒ 失败表现为「**被测程序输出了非法 JSON**」，**会被读成被测物的缺陷**。修法同族：数据不经回显，直接落文件或读子进程 stdout。

### 一道从未拒绝过任何东西的门，与不存在的门，在读数上不可区分

**上一族讲「规则能不能被省略」（尺子）；这一条讲「门能不能被证明连着」。两把尺子各量一半，任一把单独用都会给出「已处理」的假读数** —— 一道门可以机械化程度很高（无法省略）却**根本没接上**。

> **门上线时必须同时存在一次能让它开火的构造，不得事后补。**
> **记账口径：零命中不得记为「干净」，只能记为「未回答」**，除非该构造存在。

**「不得事后补」这一句是全条的重量所在**，而它来自一次自伤：一道新守卫上线时**零命中**，作者读成「代码很干净」。它确实带自测、因此过关 —— **但那是补上的，不是当时想到的**。⇒ 区分在于：**补上的自测能证明门连着，却不能证明当时的作者知道它连着。前者是代码属性，后者是记账属性，而被误读的正是后者。**

**刻意不写成「必须开过火」**：那会诱使人为了给门攒战绩而把违规样本塞进生产。要求的是**存在一次构造能让它开火** —— 它既证明门连着，又不需要真实世界配合。

**战绩必须带时点，不得记成永久属性。** 「上线即抓到两处」是一个**当时**的读数；若之后有人重构掉那两处、门回到零命中，那句话会让下一个读者以为它**仍然**被证明连着。⇒ 记 `<日期> 抓到 N 处`，而不是「已证明有效」。

**机械执行者（两层，因果那层才是承重的）**：`tests/test_gates_are_demonstrably_connected.py`。

第一层按**测试函数名的词形**挑候选。它**本身是枚举式的**，两半方向还不同：名字不匹配 ⇒ 误判「未回答」（**假阳性，方向安全**）；名字带 `reject` 而其实只测放行 ⇒ 误判「已回答」（**假阴性，方向不安全**）。⇒ **词形匹配只作必要条件，不得作充分条件。**

第二层给出**因果**证据：

> **把门拿掉，测试必须变红。**

它**不是新发明，而是本条通则的字面可执行形式** —— 通则说「有门」与「没门」在读数上不可区分，那么证明二者**可**区分的唯一办法就是**真的把门拿掉，看读数变不变**。它不依赖任何命名、任何断言写法、任何作者的表达习惯，所以它是**形态式**的。（**不要**改成「读断言写法」：那只会把枚举从函数名搬到 `assertRaises` / 非零码 / 某标记这类清单上，又是一张会被下一格推翻的表。）

变异一律在抛弃目录里做，**绝不改真实工作树**。它自己还带两道护栏，都是**被自己抓出来的、不是想到的**：

- **对照组**：同一批候选测试在**未变异**的沙箱里必须是绿的。少了它，「测试本来就红」会让变异那条**因为错误的原因通过** —— 实测抓到过一次（沙箱缺产品树的一部分，候选测试因**缺文件**而红）。
- **runner-fault**：`-k` 选中 **0 个**测试时 unittest 仍以 **0** 退出 ⇒ 「没变红」会被读成「门没连着」，而真相是**什么都没跑**。⇒ 选中 0 个一律判为**跑器故障**，不作为关于门的读数（与 `probe.py` 的 `probe-suspect` 同源：**关于跑器的读数不得记进关于被测物的账**）。

还有一处**桩本身**的陷阱值得记：桩若在**模块层**写 `sys.exit(0)`，import 时抛的 `SystemExit(0)` 会穿过 unittest 的加载机制、让**整个测试进程以 0 退出** ⇒ 变异不可见。桩必须只在 `__main__` 下退出。

### 枚举式检查在结构上弱于形态式检查

> **枚举式检查**（列出所有已知结局 / 已知危险形态）的覆盖面**等于作者的想象力**；
> **形态式检查**（禁止某一种写法、要求某一种性质）**不依赖它**。
> ⇒ 两者并存时，**形态式那条才是承重的**。

它的来源是一条更具体的观察：**一条只在「你已经想对了」时才生效的检查，防不住「没想到」这一类。** 实例就在上面那对检查里 —— 「同一脚本内 `EXIT_*` 两两不同」只在作者**已经想到这里有个结局**时才起作用，而那次失败恰恰是**根本没想到有个结局**；而「不得用字符串退出」不需要知道有哪些结局。

**本仓已落的门按这把尺子分类（写下来，因为分类本身会改变下一次的写法）**：

| 门 | 类 | 后果 |
|---|---|---|
| 投递失败形态表的逐格 fail-closed | **枚举式** | 能被「没想到的第 n 格」绕过 —— 这也正是那张表的形态集合被推翻五次的原因 |
| 「空正文拒绝发送」 | **形态式** | 不需要预先知道正文可能怎样被吞 |
| 「字段存在但值不可识别必须拒绝」 | **形态式** | 不需要预先知道有哪些合法值 |
| 「`EXIT_*` 常量两两不同」 | **枚举式** | 只守「已经想到有这个结局」的情形 |
| 「不得用字符串退出」 | **形态式** | 守「根本没想到这里有个结局」 |
| 泛化边界的同步 allowlist | **形态式**（白名单：没被看到 → 进不去）| 与[白名单通则](#allowlist-over-denylist通则门问的是谁批准了不是它有什么问题)同源 |

**两条护栏,缺了它们这条判据会被误用（两条都来自复核，方向相反）**：

1. **枚举式不是「不该有」，是「不该承重」。** 一张枚举表作为**诊断索引**有实在价值 —— 它告诉人**该往哪看**，而那份内容通常是真实事故换来的。错的是把它当**判定依据**。⇒ 处置**不是删掉枚举**，是**降级为索引、判定交给形态式那条**。**这一点不写清，下一个人会去删表，然后丢掉那份诊断经验。**
2. **穷尽划分 ≠ 枚举。** 一组对**结局空间的完备划分**不是枚举式的，哪怕它看起来也是一张清单。判据：
   > **新出现一种情况时，它是落进某个既有格子，还是需要新开一格？前者是划分，后者是枚举。**
   例：`probe.py` 的四个结局（positive / negative / inconclusive / probe-suspect）是**划分** —— 任何新情况都落进其中之一；而投递失败形态表是**枚举** —— 新病因需要新开一格（已开过五次）。**没有这条护栏，「枚举式弱」会被拿去否定所有分类工作，包括那些本来就完备的。**

⇒ **写任何新门时先问它属于哪一类。** 只有枚举式一条时，缺口清单里必须写明「本门的覆盖面等于作者当时的想象力」，不得写成「已覆盖」。

**一次用这条判据做出的可证伪预测（记录在案，因为预测比事后解释更能检验判据）**：pane 自识别门的那几格 fail-closed 条件**是枚举式的** ⇒ 预测它**会被下一个没想到的形态绕过**，且那是结构必然。据此**当时就配了形态式兜底**（「凡自识别证据不足以唯一确定该 pane ⇒ 拒绝写入」——不列举冲突形态，只要求唯一性证明），原有各格**降级为诊断索引**。
**这个预测两种结果都在检验判据**：若那道门后来真被一个新形态绕过，判据得一分；若配了形态式兜底之后再没被绕过，判据也得一分。⇒ **正因为两种结果指向同一件事，它值得在实证之前就做。**

**一条硬判据（来自那个载体**在自己身上**触发的一次，本篇少见的「门抓到造门者」记录）**：

> **任何工具的错误出口都不得复用它的结论出口**，哪怕两者在数值上「碰巧」都表示不成功。

实测：`probe.py` 的 control 文件不可读时，原实现走 `SystemExit("<字符串>")`，而字符串形态**静默退出 1** —— 正是它表示「否定，已佐证」的那个码。⇒ **一处路径笔误会以「一条确证的发现」的形态返回。**

**机械执行者**：`tests/test_exit_outlet_discipline.py`，两条检查各守那次失败的一半 ——（i）同一脚本内 `EXIT_*` 常量**两两不同**（守将来：有人给新结局挑了一个已占用的值）；（ii）**不得出现 `sys.exit("<字符串>")` / `SystemExit("<字符串>")`**（守当时那个成因：字符串形态不声明码，它**变成 1**）。第（ii）条更吃劲，因为第（i）条只在作者**已经想到这里有个结局**时才起作用，而那次失败恰恰是**根本没想到**。**它一上线就在另一个脚本里抓到两处真违例。**

**一条相关但方向相反的记账纪律：事后复盘捞回来的保护，不得记成设计意图。** 实测形态：一个为了「让 commit 干净」而养成的**具名逐文件暂存**习惯，在当事人**完全不知道另一层已失效**的情况下，独立挡住了一次「以自己的名义成为别人删除的作者」的危害。⇒ 这类保护**无法通过分析设计意图发现**，只能事后复盘发现。它**该进**缓解措施清单（因为它确实有效且不靠人记得），但**不得**反过来写成「设计上已考虑」——那会让下一个人以为该危害已被设防。

**同族的一个变体：失效的步骤被有效的步骤掩盖。** 实测形态是「一条不存在的子命令」后面紧跟「一条真正起作用的具名命令」，合并流使前者的失效**永久不可见**（12 次落盘每次都 `1 file changed`，工作全由后者完成）。⇒ **凡一条流程里有两步声称做同一件事，必须能分别证明每一步都在做事**；否则那条流程只证明了**至少一步**在做事，而它读起来像证明了两步。

### 状态表里的「已落」必须附一条独立读数

**上一条讲记录少了一维；这一条讲记录的那一格是假的。**

跨 session 协作靠**状态表**（gap 队列、dashboard、台账、进度汇报）分配注意力。而一格标成「已落 / ✅ / done」的**通常来源是 owner 的自报**，读表方**没有第二条证据**。

> **实测:同一天里两个方向都发生了。** 一次**多报** —— 一封汇报信里写「我按你给的方案落」（**计划态**）被读表方读成完成态并标 ✅；一次**少报** —— 一封更正信写「已落的**只有** X 和 Y」，把范围写成了完备声明，漏掉一项**其实已合入**的。
>
> ⇒ **错的不是某一方,是自报这个信道本身 —— 它双向都会错。** 这是本条与「先怀疑叙述」那类规则的分界：那些假定**有一个读数**在与叙述冲突；这里**根本没有读数,只有叙述**，连冲突都不存在。**缺的不是怀疑,是「没有读数时不得标已落」这道门。**

**代价不对称,所以 fail-safe 方向是单侧的**：

| 错法 | 后果 |
|---|---|
| **假的「已落」** | 该项**从所有人视野里消失** —— 没人再去做，也没人再去问 |
| **漏记 / 保守地不标** | **还会被人再提起**（它仍在待办面上） |

⇒ **判不准时不标「已落」。** 与本篇其它 fail-safe 同向：不可逆的那一侧要更保守。

**机械产物（否则本条自己就是装饰）**：

> 状态表任何一格标「已落」，**必须同格附一条读表方自己能复核的读数** —— commit id / `grep` 命中 / 文件存在 / 测试计数之一。**不接受**「owner 说已落」「已合并」「已交付」作为唯一依据。

**它与「制品到位 ≠ 路径生效」是两级,别合并**：那一条说**制品在了,但路径未必生效**；这一条更低一级 —— **连制品都可能不在**，而表上已写着 ✅。⇒ 一格「已落」要能同时答两问：**（i）** 制品在吗（本条的读数）；**（ii）** 路径生效了吗（那一条的[层](#门的回归必须端到端且测试的结构必须与真实调用路径同构)）。只答第一问的仍要在缺口栏写明第二问未答。

**引用一个值之前必须先持有它 —— 这是顺序约束，不是自觉约束。** 实测：一封**通篇在讲本条规则**的信里，引用了一个**当时并不存在**的 commit id —— 因为信是在 commit 之前写的，那个位置需要一个值，于是被填了一个。正文其余部分完全自洽，而被编造的恰是**具体值**那一类（与[发送侧损坏的签名](./agenttui-registry.md#3-投递契约规则-18)同形：错误专挑读数、路径、标识符）。

> **凡记录要引用一个由动作产生的值（commit id、测试计数、行号、hash），必须先做那个动作、再把真实值读出来填入。顺序不可颠倒。**

**为什么顺序约束优于自觉约束**（这是本条的重量，不是那次失误）：顺序颠倒时**没有值可读**，于是**缺失是可见的**；而编造**不可见**。⇒ 它把一个不可见的失败模式换成一个可见的。按[机械化尺子](#判够不够机械的尺子这条把上面那个元结论变成可施加的判据)，「下次注意」在这里是**负**档 —— 先写文后动手**更省事**，所以时间会稳定地把人推向编造那一侧。

**给汇报方的对应纪律（缺口在语言侧，不只在读表侧）**：进度信里描述尚未完成的事，**不得使用可被读成完成态的措辞**（「我按 X 落」「已按 X 处理」在一封汇报进度的信里天然被读成过去式）。写成「**已排入,未落**」并给出阻塞原因。**双方各修一半,单侧修不掉** —— 读表方加了门，措辞仍会让门收到一条假读数。
...
**判据的一个加强（来自一次「测试通过、而且断言的字符串确实出现了」的实测）**：

> **断言命中 ≠ 命中来自你以为的那一层。**

实测形态：一批新测试**假设**「正文解析先于仓根解析」，于是不给仓根参数；结果两条测试**通过了，而且它们断言的字符串确实出现在输出里**——只是来自**仓根拒绝**那一层，被断言的那段代码**根本没被执行**。实测层序与假设相反。⇒ 这比本节原有两个实例更隐蔽：那两个是「测了但没测到门」，这个是**测试全绿、断言命中、而命中来自别处**。

**机械做法**：当同一个字符串**可能由多层产生**时，断言必须**锚定到层** —— 构造只有目标层能满足的前置（例如先让更早那几层全部通过），或断言一个**只有目标层会产生**的标记。否则它证明的只是「**系统某处会说这句话**」。**实例贴着它防的那段代码写**（测试的 docstring）比写进规范更可能被读到；规范这里只留判据。

...
**机械执行者**：`tests/test_gates_are_demonstrably_connected.py` —— 对**铺出去的每一个门**要求「至少有一处测试既提到它、又含一个会开火的测试」。它**自己**也带一次构造样本（只有放行侧测试的门必须被判为「未回答」），理由是若只测真实文件，它永远不会开火，于是它自己就成了它要防的那种门。
